...
Private Type TMain
    PADDING_CHAR As String
End Type

Private this As TMain

Public Property Get PADDING_CHAR() As String
    PADDING_CHAR = this.PADDING_CHAR
End Property

Public Property Let PADDING_CHAR(ByVal value As String)
    this.PADDING_CHAR = value
End Property

'@Ignore AssignedByValParameter
Public Function StringFormat(ByVal mask As String, ByRef values As Variant) As String
    ...
    regex.Global = True
    Set matches = regex.Execute(mask)
    ...
    useLiteral = Strings.StartsWith("@", mask)
    If useLiteral Then mask = Right$(mask, Len(mask) - 1) 'remove the "@" literal specifier

    If Not useLiteral And Strings.Contains(mask, "\\") Then _
                                                                mask = Replace(mask, "\\", Chr$(27))

    If matches.Count = 0 And mask <> vbNullString And UBound(values) = -1 Then
    ...
    Dim v As String
    Dim p As String
    'i: iterator; v: value; p: placeholder
    Dim alignmentGroup As String
    Dim alignmentSpecifier As String
    ...
    For i = 0 To matches.Count - 1
        ...
        If Strings.Contains(thisString, ":") Then
            ...
            Dim formatGroup As String
            Dim precisionSpecifier As Long

            Dim formatSpecifier As String
            Dim precisionString As String
